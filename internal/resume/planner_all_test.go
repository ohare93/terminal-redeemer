...
func TestAllCarriesPersistedColumnOccupancyFromHeadlessStickyCheckpoint(t *testing.T) {
	...
	checkpoint.Recovery.Sessions[0].CapturedColumnOccupied = true
	...
	plan := NewPlanner(PlannerConfig{}).BuildAll(selection, current, catalog, AllOptions{Now: now, MaxAge: time.Hour})
	item := assertSessionStatus(t, plan, "hidden", StatusReady)
	if !item.CapturedColumnOccupied {
		t.Fatalf("hidden non-terminal stack occupancy was not carried into plan: %#v", item)
	}
	if reason := stackedReason(&plan, []attachedTarget{{item: 0}}); !strings.Contains(reason, "stacked rows are unsupported") {
		t.Fatalf("persisted occupancy did not degrade ordering: %q", reason)
	}
}
